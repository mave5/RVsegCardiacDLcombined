...
hiddenSizeL1 = 750;     % number of hidden units 
hiddenSizeL2=750;
...
% random permutation
p = randperm(size(t_Iroi,3));
t_Iroi_s=t_Iroi(:,:,p);
t_y_endo_s=t_y_endo(:,:,p);

train_input=sampleIMAGES(t_Iroi_s,patchsize);
train_labels=sampleIMAGES(t_y_endo_s,patchsize);
...
testI=t_Iroi+randn(size(t_Iroi));
...
%% save results
filename=['matFiles/SimResults/Suffle_V_',num2str(patchsize),'_RVseg_H1_',num2str(hiddenSizeL1),'_H2_',num2str(hiddenSizeL1),'_rho1_',num2str(sparsityParam1*100),'div100','_rho2_',num2str(sparsityParam2*100),'div100'];
save (filename);
