...
nLarge2x=26;
%nLarge2x=numFiles;
for k=nLarge2x+1:2:numFiles
    % k1=k/2
    k1=floor((k-nLarge2x)/2)+1;
...
for k=1:z_max
    ...
    figure(2)
    subplot(3,ceil(z_max/3),k)
    yROI(:,:,k)=poly2mask(wx_corners,wy_corners,x_max,y_max);
...
I=IMAGES(:,:,1:nLarge);
fname1=['matFiles/',imset,'/SmallContours'];
if exist(fname1,'dir')==0
    mkdir (fname1);
end
filename=[fname1,'/patient',pnstr];
