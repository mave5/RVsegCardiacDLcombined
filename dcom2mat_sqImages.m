...
%% patient number
large_slices_per_patient=[8,10,8,7,10,8,9,11,10,10,10,10,10,9,8,8];
patient=16;
nLarge2x=2*large_slices_per_patient(patient);
%nLarge2x=numFiles;

disp(['patient:',num2str(patient),'    large slice:',num2str(nLarge2x/2),'      save enable:',num2str(save_ena)])
% region of interest
Mroi_x=171;
Mroi_y=171;
...
% here we colect names of large contours only
for k=1:2:nLarge2x
    % k1=k/2
    k1=floor(k/2)+1;
...
% make image square size, some images are 256*216 or 216*256.
% we want image of size 256*256
...
I=IMAGES(:,:,1:nLarge);
I_sq=IMAGES_sq(:,:,1:nLarge);
filename=['matFiles/',imset,'/LargeContours','/patient',pnstr];

if save_ena==1
save (filename, 'I','I_sq' ,'yROI','Iroi','y_epi','y_endo','contours_endo','contours_epi',...
