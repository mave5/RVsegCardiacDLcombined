...
hiddenSizeL1 = 500;     % number of hidden units 
hiddenSizeL2=500;
...
load matFiles/TrainingSet/all_patients; 
train_input=sampleIMAGES(t_Iroi,patchsize);
train_labels=sampleIMAGES(t_y_endo,patchsize);
...
for k=1:size(yRVhr,3)
    ...
    yRV_hce(:,:,k)=edit_prior_shape(t_Iroi(:,:,k),yRV_hc(:,:,k),0);
end
...
for k=60:89
    ...
    contour(yRV_hc(:,:,k),[0 0],'y','LineWidth',2);    
    contour(yRV_hce(:,:,k),[0 0],'b','LineWidth',2);    
end
...
%% save results
filename=['matFiles/workspace/InputSize_',num2str(patchsize),'_RVseg_H1_',num2str(hiddenSizeL1),'_RVseg_H2',num2str(hiddenSizeL2),'_rho_',num2str(sparsityParam1*100),'div100'];
save (filename);

% save network trained parameters for further use
filename2='matFiles/workspace/DLparams';
%save (filename2,'stackedAEOptTheta','inputSize','hiddenSizeL1','hiddenSizeL2','outputSize','netconfig');
